        ...
        if os.path.exists(cache_path):
            try:
                with open(cache_path, 'rb') as f:
                    ...
                    ValueError):  ###如果存在，则尝试从缓存文件中加载 partition_node_ids 和 partition_offset。如果加载失败，会引发异常并提示缓存文件内容无效
                raise DGLError(
...
class Edge_partition_sampler_new(hrdf_Sampler):
    def __init__(self, g, k, cache_path, partition_file_path=None, prefetch_ndata=None,
                 prefetch_edata=None, output_device=None):
    ...
    def sample(self, g, partition_ids):
        ...
        # 使用zip和*操作符来解包元组
        src, dst = zip(*merged_set)

        # 将src和dst转换为张量，以便进行批量操作
        src_tensor = torch.tensor(src, dtype=torch.int64)
        dst_tensor = torch.tensor(dst, dtype=torch.int64)

        # 使用DGL的批量边ID查找
        edge_ids = g.edge_ids(src_tensor, dst_tensor)

        # 创建子图
        sg = dgl.edge_subgraph(g, edge_ids, relabel_nodes=True, output_device=self.output_device)
...
class Edge_partition_sampler_id(hrdf_Sampler):
    ##构造函数，g图对象，k是分区，cache_pat缓存路径
    def __init__(self, g, k, cache_path, partition_file_path=None, prefetch_ndata=None,
                 prefetch_edata=None, output_device=None):
        super().__init__(g, k, cache_path, partition_file_path,
                         prefetch_ndata, prefetch_edata, output_device)
    ...
    def sample(self, g, partition_ids):
        ...
        i = partition_ids[0]
        print("i", i)
        if not self.blocks[i]:##self.blocks[i]是存储图分区的列表
            raise ValueError("No valid blocks found for the given partition_ids.")
        # 创建子图
        # print("self.blocks[i]",self.blocks[i])
        blocks = self.blocks[i]
        sg = dgl.edge_subgraph(g, blocks, relabel_nodes=True, output_device=self.output_device)
        # # 打印子图的基本信息
        # print("子图信息:", sg)
        # node_ids = sg.ndata['_ID']
        # print("子图原id:", node_ids)
        # print("子图新id", sg.nodes())
        #
        # # 打印子图的节点数量和边数量
        # print("子图节点数:", sg.num_nodes())
        # print("子图边数:", sg.num_edges())
