...
dataset = dgl.data.CoauthorCSDataset(raw_dir='/23085411005/23085411005/hdrf_hy_pycharm/data/CoauthorCSDataset')
graph = dataset[0]
...
dataloader = dgl.dataloading.DataLoader(  #####边划分
    ...
    util.Sampler2.Edge_partition_sampler_id(
        ...
        cache_path=f'/23085411005/23085411005/hdrf_hy_pycharm/vertex_partitions/hdrf/coa/76_bian',
...
# # 创建一个全零的 2D 张量，形状为 (节点数, 128)
# num_nodes = graph_hidden1.number_of_nodes()  # 获取节点数量
# zero_feats = torch.zeros(num_nodes, num_hidden)  # 创建一个节点数量 x 128 的全零张量
# # 将节点特征设置为全零
# graph_hidden1.ndata['feat'] = zero_feats.to(device)
# graph_hidden1.ndata['_ID'] = torch.arange(num_nodes).to(device)
#
# graph_hidden2 = copy.deepcopy(graph).to(device)
# num_nodes = graph_hidden2.number_of_nodes()  # 获取节点数量
# zero_feats = torch.zeros(num_nodes, num_hidden)  # 创建一个节点数量 x 128 的全零张量
# # 将节点特征设置为全零
# graph_hidden2.ndata['feat'] = zero_feats.to(device)
# graph_hidden2.ndata['_ID'] = torch.arange(num_nodes).to(device)
# # print(graph_hidden.ndata['_ID'])
...
for e in range(num_epochs):
    ...
    #     device = next(model.parameters()).device
    #     # 初始化tensor来存储预测结果和标签
    #     y_hat_tensor = torch.zeros(num_nodes, dataset.num_classes).to(device)
    ...
    #
    #         # 获取新加入的邻居节点
    #         neighbors_set = set(neighbors.cpu().tolist())
    #         sub_nodes_set = set(sg_nodes.cpu().tolist())
    #         new_neighbors_set = neighbors_set - sub_nodes_set
    #         new_neighbors = torch.tensor(list(new_neighbors_set), device=device)
    #
    #         # 获取节点标签和掩码，确保都在正确的设备上
    #         y = expanded_subgraph.ndata["label"].to(device)
    #         m_val = expanded_subgraph.ndata["val_mask"].bool().to(device)
    #         m_test = expanded_subgraph.ndata["test_mask"].bool().to(device)
    #         sg_node_ids = expanded_subgraph.ndata['_ID'].to(device)
    #
    #         # 排除新加入的邻居节点
    #         mask = torch.isin(sg_node_ids, new_neighbors)
    #         m_val = m_val & ~mask
    #         m_test = m_test & ~mask
    #
    #         # 获取模型预测
    ...
    #
    #         # 更新预测结果和标签
    #         for i, node_id in enumerate(sg_node_ids):
    #             if not mask[i]:  # 只统计非新邻居节点的预测
    #                 node_id = node_id.item()
    #                 y_hat_tensor[node_id] += y_hat[i]
    #                 val_false_tensor[node_id] = m_val[i]
    #                 test_false_tensor[node_id] = m_test[i]
    #                 y_tensor[node_id] = y[i]
    #
    #     # 直接计算验证集和测试集准确率，不进行平均
    #     val_acc = MF.accuracy(y_hat_tensor[val_false_tensor],
    #                           y_tensor[val_false_tensor],
    #                           task="multiclass",
    #                           num_classes=dataset.num_classes)
...
###cora_results
with open(f"/23085411005/23085411005/hdrf_hy_pycharm/result/coa_lmc/zitu_qiuhe/coa_hdrf_mb_76_3_128_lr0.001.csv", "w",
